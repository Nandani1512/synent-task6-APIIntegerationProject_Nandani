...
const REGION_SYSTEM_INSTRUCTION = [
  ...
  "4. LIMIT TO 1 PAGE (2-3 PROJECTS MAX): The final resume must fit on a single",
  "   page. Output AT MOST 2 to 3 project entries. If the original had more,",
  "   aggressively trim the weakest ones. (If the original region is empty,",
  "   instead ADD the 2-3 best-matching repos.)",
  ...
  "   \"Responsible for\", \"Supported\"). Avoid vague filler, include real metrics",
  "   and data for ATS optimization where possible, and write \"and\" rather than \"&\".",
...
function buildRegionPrompt(repos, jobDescription, originalRegion) {
  ...
  return [
    ...
    "replace weak ones with more relevant repos that have detailed READMEs,",
    "limit to 2-3 projects max (1 page), match the LaTeX format exactly, and wrap",
